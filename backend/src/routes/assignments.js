...
router.get(
  ...
  wrapAsync(async (req, res) => {
    ...
    const payload = await AssignmentsDBApi.findAll(req.query);
    if (filetype && filetype === 'csv') {
      const fields = ['id', 'title', 'description', 'due_date'];
      const opts = { fields };
      try {
...
router.get(
  '/:id',
  wrapAsync(async (req, res) => {
    const payload = await AssignmentsDBApi.findBy({ id: req.params.id });
