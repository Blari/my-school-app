...
module.exports = class ClassesDBApi {
  static async create(data, options) {
    ...
    await classes.setTeacher(data.teacher || null, {
      transaction,
    });

    await classes.setStudents(data.students || [], {
      transaction,
    });
  ...
  static async findBy(where, options) {
    ...
    output.teacher = await classes.getTeacher({
      transaction,
    });

    output.students = await classes.getStudents({
      transaction,
    });
  ...
  static async findAll(filter, options) {
    ...
    if (filter) {
      ...
      if (filter.teacher) {
        var listItems = filter.teacher.split('|').map((item) => {
          return Utils.uuid(item);
        });

        where = {
          ...where,
          teacherId: { [Op.or]: listItems },
        };
      }
    ...
    let { rows, count } = options?.countOnly
      ...
      : await db.classes.findAndCountAll({
          where,
          include,
          distinct: true,
          limit: limit ? Number(limit) : undefined,
