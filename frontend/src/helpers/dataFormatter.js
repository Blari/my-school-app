...
export default {
  ...
  assignmentsManyListFormatter(val) {
    if (!val || !val.length) return [];
    return val.map((item) => item.title);
  },
  assignmentsOneListFormatter(val) {
    if (!val) return '';
    return val.title;
  },
  assignmentsManyListFormatterEdit(val) {
    if (!val || !val.length) return [];
    return val.map((item) => {
      return { id: item.id, label: item.title };
    });
  },
  assignmentsOneListFormatterEdit(val) {
    if (!val) return '';
  ...
  teachersManyListFormatter(val) {
    if (!val || !val.length) return [];
    return val.map((item) => item.first_name);
  },
  teachersOneListFormatter(val) {
    if (!val) return '';
    return val.first_name;
  },
  teachersManyListFormatterEdit(val) {
    if (!val || !val.length) return [];
    return val.map((item) => {
      return { id: item.id, label: item.first_name };
    });
  },
  teachersOneListFormatterEdit(val) {
    if (!val) return '';
    return { label: val.first_name, id: val.id };
  },
