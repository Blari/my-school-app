...
import { loadColumns } from './configureAssignmentsCols';
...
const TableSampleAssignments = ({
  ...
  useEffect(() => {
    if (assignmentsNotify.showNotification) {
      notify(
        assignmentsNotify.typeNotification,
        assignmentsNotify.textNotification,
      );
    }
  }, [assignmentsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/assignments/assignments-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {assignments && Array.isArray(assignments) && !showGrid && (
        <ListAssignments
...
export default TableSampleAssignments;
