...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/grades/gradesSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureGradesCols';
...
const TableSampleGrades = ({
  ...
  useEffect(() => {
    if (gradesNotify.showNotification) {
      notify(gradesNotify.typeNotification, gradesNotify.textNotification);
    }
  }, [gradesNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/grades/grades-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {grades && Array.isArray(grades) && !showGrid && (
        <ListGrades
          grades={grades}
          loading={loading}
...
export default TableSampleGrades;
